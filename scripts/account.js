window.jQuery( function ( $ ) {
	...
	$( '.woocommerce-MyAccount-content__back-button' ).on( 'click', () => {
		if ( window.innerWidth < BREAKPOINT ) {
			$( '.woocommerce-MyAccount-content' ).slideUp( () => {
				$(
					'.page-header, .woocommerce-MyAccount-navigation'
				).slideDown();
			} );
		}
	} );

	$( '.woocommerce-MyAccount-navigation .is-active' ).on(
		'click',
		( event ) => {
			if ( window.innerWidth < BREAKPOINT ) {
	...
	$( window ).on( 'resize', () => {
		if ( window.innerWidth >= BREAKPOINT ) {
			$(
				'.woocommerce-MyAccount-content, .woocommerce-MyAccount-navigation, .page-header'
			).removeAttr( 'style' );
		}
	} );
} );
